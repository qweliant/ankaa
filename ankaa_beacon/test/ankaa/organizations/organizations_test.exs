defmodule Ankaa.OrganizationTest do
  ...
  describe "organizations" do
    ...
    test "list_available_colleagues/2 returns colleagues in the same org" do
      ...
      %{user: outsider} = AccountsFixtures.doctor_fixture()

      %{patient: patient} = AccountsFixtures.patient_fixture()
      ...
      colleagues = Patients.list_available_colleagues(doctor_a, patient.id)
    ...
    test "list_available_colleagues/2 filters out already assigned members" do
      ...
      {:ok, _} = Patients.create_patient_association(doctor_b, patient, "doctor", :admin, :doctor)
    ...
    test "list_available_colleagues/2 does not add user to a new organization via care network association" do
      ...
      %{patient: patient, user: patient_user} = AccountsFixtures.patient_fixture()
      {:ok, _} = Patients.create_patient_association(doctor, patient, "doctor", :admin, :doctor)

      {:ok, _} =
        Patients.create_patient_association(nurse, patient, "nurse", :contributor, :nurse)

      colleagues = Patients.list_available_colleagues(doctor, patient.id)
